...
static int test = RESTORE_LEDS; // cod
/*
...
static void my_timer_func(struct timer_list *ptr)
{
        ...
        //printk(KERN_INFO "kbleds: pstatus = %i\n", *pstatus);
...
static int __init kbleds_init(void)
{
        ...
        example_kobject = kobject_create_and_add("kleds", kernel_kobj);
        if(!example_kobject)
                return -ENOMEM;
        error = sysfs_create_file(example_kobject, &ko_attr.attr);
        if (error)
            printk(KERN_INFO "kbleds: failed to create file in /sys/kernel/kleds\n");
